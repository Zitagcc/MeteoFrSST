...
def gen_platform(exe_name, topo, ncpu, cores_per_node, bandwidth, latency, params, amm="amm1", model=dict(congestion='pisces', node='simple', memory='simple', nic='pisces', switch='pisces'), allocation="first_available", indexing="block", routing='minimal', stat=0):
...
def submit_smpi(work_dir,exe_file,ntasks,nnode=1,cores_per_node=1,wait=None,max_wait=1440,command_args='',dependency=None,sid=''):
    if dependency is None:
        dependency = ''
    else:
        dependency = """
...
""" % (dependency['chk_file'], dependency['chk_freq'])
    ...
    for ntask in ntasks:
        ...
        with open(cmd_file, 'wt') as fcmd:
            fcmd.write("#!/bin/sh\n")
            fcmd.write("%s/shm_mgr.x %s/%08d %d -1\n" % (work_dir, work_dir, ntask, ntask)) # -1=MPI_PROC_NULL in SST/MACRO
            fcmd.write("GO_MBLOCK_SHMID=`cat %s/%08d/GO_MBLOCK_SHMID`\n" % (work_dir, ntask))
            fcmd.write("%s/%s -s $GO_MBLOCK_SHMID  -f %s/%08d/parameters.ini %s\n" % (work_dir, exe_file, work_dir, ntask, command_args))
            fcmd.write("%s/shm_mgr.x $GO_MBLOCK_SHMID\n" % (work_dir))
...
""" % (exe_name, nodes, cores_per_node, partition, reservation, ntasks[0], dependency, mac_file, mac_file))
        if len(ntasks) == 1:
            for ntask in ntasks:
                fbat.write("mpirun -machinefile %s -np $nn %s/shm_mgr.x %s/%08d %d -1\n" % (mac_file, work_dir, work_dir, ntask, ntask)) # -1=MPI_PROC_NULL in SST/MACRO
                fbat.write("GO_MBLOCK_SHMID=`cat %s/%08d/GO_MBLOCK_SHMID`\n" % (work_dir, ntask))
                fbat.write("mpirun -machinefile %s -np %d %s/%s -s $GO_MBLOCK_SHMID  -f %s/%08d/parameters.ini %s\n" % (mac_file, nnode*cores_per_node, work_dir, exe_file, work_dir, ntask, command_args))
                fbat.write("mpirun -machinefile %s -np $nn %s/shm_mgr.x $GO_MBLOCK_SHMID\n" % (mac_file, work_dir))
        else:
            for ntask in ntasks:
...
def run_cpus(work_dir, exe_file, nx, ny, upper, nstep, njob, wait, restart, allocation='first_available', indexing='block', topo=None, bandwidth=None, latency=None, params=None, nnode=1, cores_per_node=1, routing='minimal', stat=0):
    if (njob < 0):
        njobs = [1,2,8,16]
        ntasks = []
    else:
        ...
        if (ncpu > old_ncpu):
            ...
            if (ncpu >= restart):
                ...
                if njob == 0:
                    ...
                    # submit
                    chk_file = submit_smpi(work_dir, exe_file, [ncpu], max_wait=7200, nnode=nnode, cores_per_node=cores_per_node)
...
if __name__ == '__main__':
    ...
    if len(argv) < 4:
        ...
        nx, ny, upper = map(lambda x: int(x), argv[1].split(':'))
        nstep = int(argv[2])
        wait = int(argv[3])
        if len(argv) > 10:
    ...
    if len(argv) > 10:
        run_cpus(work_dir, exe_file, nx, ny, upper, nstep, njob, wait, restart, allocation=allocation, indexing=indexing, topo=topo, bandwidth=bandwidth, latency=latency, params=params, nnode = nnode, cores_per_node=cores_per_node, routing=routing, stat=stat)
    else:
