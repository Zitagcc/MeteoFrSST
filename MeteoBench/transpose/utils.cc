...
void mfft(double L, int N, int M, int stride, int dist, double *ibuf, double *obuf, fftw_complex *cbuf) {
    double factor = 2.0L*PI/(L*N); // N for normalization
    fftw_plan p;

    // from physical space to spectral space
    p = fftw_plan_many_dft_r2c(1, &N, M, ibuf, NULL, stride, dist, cbuf, NULL, 1, N/2+1, FFTW_ESTIMATE);
    fftw_execute(p);
    fftw_destroy_plan(p);

    // calculate the first derivative in spectral space
    // N % 2 == 0: k = [0:N/2-1, 0, -N/2+1:-1]
    // else: k = [0:N/2-1, 0, -N/2:-1]
    for (int j = 0; j < M; j++) {
        for (int i = 0, idx = j * (N/2+1); i <= N / 2; i++, idx++) {
            double t = cbuf[idx][0];
            cbuf[idx][0] = cbuf[idx][1] * (-i) * factor;
            cbuf[idx][1] = t * i * factor;
        }
    }

    // from spectral space to physical space
    p = fftw_plan_many_dft_c2r(1, &N, M, cbuf, NULL, 1, N/2+1, obuf, NULL, stride, dist, FFTW_ESTIMATE);
    fftw_execute(p);
    fftw_destroy_plan(p);
}
...
#if !defined(_NOMEM_)
    if (dm->nx <= 0 || dm->ny <= 0) return;

    fftw_complex *cbuf;
    double *F, *G;
    if (dir == 'x') {
        cbuf = (fftw_complex *)fftw_malloc(sizeof(fftw_complex) * (dm->nx/2+1) * dm->ny);
        for (int k = dm->kts; k <= dm->kte; k++) {
          F = fld +   INDEX(dm->its - dm->ims, dm->jts - dm->jms, k - dm->kms, dm->mx, dm->mslice);
          G = deriv + INDEX(dm->its - dm->ims, dm->jts - dm->jms, k - dm->kms, dm->mx, dm->mslice);
          mfft(L, dm->nx, dm->ny, 1, dm->mx, F, G, cbuf);
        }
        fftw_free(cbuf);
    }else if (dir == 'y') {
      cbuf = (fftw_complex *)fftw_malloc(sizeof(fftw_complex) * (dm->ny/2+1) * dm->nx);
      for (int k = dm->kts; k <= dm->kte; k++) {
        F = fld +   INDEX(dm->its - dm->ims, dm->jts - dm->jms, k - dm->kms, dm->mx, dm->mslice);
        G = deriv + INDEX(dm->its - dm->ims, dm->jts - dm->jms, k - dm->kms, dm->mx, dm->mslice);
        mfft(L, dm->ny, dm->nx, dm->mx, 1, F, G, cbuf);
      }
      fftw_free(cbuf);
    }
#endif
